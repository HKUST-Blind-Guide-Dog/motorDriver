...
void Motor::setZero()
{
    ...
    commandBuffer[2] = 8; // data size
    // set zero
    commandBuffer[3] = 0x64;
    commandBuffer[4] = commandBuffer[5] = commandBuffer[6]= commandBuffer[7]= commandBuffer[8] = commandBuffer[9] = commandBuffer[10] = 0x00;
    // serial communicatiom
    serial_crc16(commandBuffer, bufferSize - 2, &commandBuffer[11], &commandBuffer[12]); // write the corresponding crc
    serial_write(serialProtocol.serialPort, bufferSize, commandBuffer);
    serial_read(serialProtocol.serialPort, bufferSize, dataBuffer);
    usleep(0.002 * 1000000);
}
...
bool Motor::setComSpeed(int speed)
{
    ...
    commandBuffer[4] = commandBuffer[5] = commandBuffer[6] = commandBuffer[7] = commandBuffer[8] = commandBuffer[9] = 0x00;
...
bool Motor::readMOtorData()
{
    ...
    uint8_t dataBuffer[bufferSize] = {0};
    usleep(0.002 * 1000000);
    int success = !serial_read(this->serialProtocol.serialPort, bufferSize, dataBuffer);
    if (success)
    {
        if (IS_DEBUG)
        {
